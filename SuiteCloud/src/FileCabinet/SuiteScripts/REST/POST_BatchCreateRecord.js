...
define(['N/record', 'N/log'], (record, log) => {
    ...
    /**
     * @TODO decide if want to make createRecordArray and createRecordDict mutually exclusive.
     * 
     ...
     * @param {Array<CreateRecordOptions>} [reqBody.createRecordArray] (optional) Method 1 — 
     * Array<{@link CreateRecordOptions}>
     * - for req in reqBody.createRecordArray
     * - - run function {@link processCreateRecordOptions}(req)
     * @param {{[K in RecordTypeEnum]?: Array<CreateRecordOptions>}} [reqBody.createRecordDict] (optional) Method 2 — 
    ...
    const post = (reqBody) => {
        ...
        let createRecordDictIsUndefined = !reqBody.createRecordDict || typeof reqBody.createRecordDict !== 'object';
        ...
        let createRecordDict = reqBody.createRecordDict || {};
        ...
        try {
            if (createRecordArray.length > 0) {
                for (let i = 0; i < createRecordArray.length; i++) {
                    ...
                    if (!result) {
                        writeLog(LogTypeEnum.ERROR, `Error Processing createRecordArray[${i}]`, 'Invalid createReq: CreateRecordOptions =  ' + JSON.stringify(createReq));
                        continue;
                    } 
                    recIdArray.push(result);
                }
            }         
            writeLog(LogTypeEnum.DEBUG, 'Object.keys(reqBody.createRecordDict).length:', Object.keys(createRecordDict).length);
            if (Object.keys(createRecordDict).length > 0) {
                for (let recordType in createRecordDict) {
                    ...
                    for (let i = 0; i < createReqArray.length; i++) {
                        ...
                        let result = processCreateRecordOptions(createReq);
                        if (!result) {
                            writeLog(LogTypeEnum.ERROR, `Error Processing createRecordDict[${recordType}][${i}]`, 'Invalid createReq: CreateRecordOptions =  ' + JSON.stringify(createReq));
                            continue;
                        } 
                        recIdArray.push(result);
                    }
     ...
     * @returns {number} `line {number}` if valid, otherwise add a new line to the sublist and return it as the new line index. 
     */
 ...
 * - Array<{@link SetSubrecordOptions}> = Array<{sublistId: string=undefined, fieldId: string, subrecordType: string, line: number=undefined, fieldDict: {@link FieldDictionary}, sublistDict: {@link SublistDictionary}}>.
 ...
 * @property {Array.<SetSubrecordOptions>} [subrecordFields]
 * - Array<{@link SetSubrecordOptions}> = Array<{sublistId: string, fieldId: string, subrecordType: string, line: number, fieldDict: {@link FieldDictionary}, sublistDict: {@link SublistDictionary}}>.
 ...
 * @property {SublistDictionary} [sublistDict] - Record<[sublistId: string], {@link SublistFieldDictionary}> = { sublistId: { textFields: Array<{@link SetSublistTextOptions}>, valueFields: Array<{@link SetSublistValueOptions}> } }.
