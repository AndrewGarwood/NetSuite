    ...
    const doDelete = (/**@type {DeleteRecordByTypeRequest} see {@link DeleteRecordByTypeRequest} */reqBody) => {
        ...
        try {
            search.create({type: recordType, filters: []}).run().each((result) => {
                ...
                if (maxDeletions && recordsToDelete.length >= maxDeletions) {
                    writeLog(LogTypeEnum.AUDIT, `Max deletions reached`, `Max deletions of ${maxDeletions} reached. Stopping search.`);
                ...
                if (canDelete(internalId, recordType, excludeOptions)) {
                    ...
                    writeLog(LogTypeEnum.DEBUG, 
                        ...
                        `'${recordType}' record ${internalId} excluded from deletion`
                    );
                    return true; // continue the search
                }
            });
        ...
        for (let i = 0; i < maxDeletionsToProcess; i++) {
            ...
            try {
                ...
                record.delete.promise({ type: recordType, id: internalId });
    ...
    function canDelete(id, recordType, excludeOptions) {
        if (typeof id !== 'number' && typeof id !== 'string') {
            writeLog(
                ...
                `id must be a number or string. received type: '${typeof id}'`, 
        ...
        if (typeof id === 'string') {
            ...
            if (isNaN(id)) {
                writeLog(
                    LogTypeEnum.ERROR, 
                    `canDelete() Invalid id for recordType '${recordType}'`, 
                    `id must be a number or a string that can be parsed to a number. received: '${id}'`,
                );
                return false;
            }
        }
